...
test('groupPagesByNotebook 按 notebook 分组且保持顺序、跳过无 notebook 页面', () => {
  const pages = [
    { _id: 'a1', collection: { profile: 'wiki', id: 'wrong' } },
    { _id: 'x' },
    { _id: 'a2' },
    { _id: 'b1' },
    { _id: 'y' }
  ...
  const pageConfigs = new Map([
    [pages[0], { collection: { profile: 'notebook', id: 'alpha' }, tags: [] }],
    [pages[1], {}],
    [pages[2], { collection: { profile: 'notebook', id: 'alpha' }, tags: [] }],
    [pages[3], { collection: { profile: 'notebook', id: 'beta' }, tags: [] }],
    [pages[4], {}]
  ]);
  const map = groupPagesByNotebook(pages, pageConfigs);
  assert.deepStrictEqual([...map.keys()], ['alpha', 'beta']);
  assert.deepStrictEqual(map.get('alpha').map(p => p.id), ['a1', 'a2']);
  assert.deepStrictEqual(map.get('beta').map(p => p.id), ['b1']);
  assert.equal(map.has(''), false);
});

test('groupPagesByNotebook 空数组返回空 Map', () => {
  const map = groupPagesByNotebook([], new Map());
...
test("Notebook 集合只保留显式菜单覆盖，不用 note_index 默认遮蔽 note Profile", () => {
  ...
  const notebooks = getNotebooksObject({
    ...
    stellar: {
      ...
      contentConfig: {
        collectionConfigs: new Map(Object.entries(data).map(([key, value]) => [key, parseCollectionConfig(value)])),
        pageConfigs: new Map()
      }
    },
