...
test('dropdown 标签渲染方向、对齐、默认展开和链接属性', () => {
  ...
  assert.match(html, /<summary class="dropdown-trigger ui-interactive" title="更多链接" aria-label="更多链接">/);
  assert.match(html, /<span>更多链接<\/span><svg class="dropdown-arrow" viewBox="0 0 24 24"/);
  assert.match(html, /<div class="dropdown-menu ui-collection" data-layout="list" data-variant="nav" data-density="compact">/);
  assert.doesNotMatch(html, /data-ui-surface/);
  assert.match(html, /class="dropdown-item ui-collection__item ui-interactive card-hover card-hover--spotlight"/);
  assert.match(html, /class="ui-icon"><svg><\/svg><\/span>/);
...
test('dropdown 标签转义标题和链接属性，并支持无图标子项', () => {
  ...
  const iconlessItem = html.match(/<a class="dropdown-item ui-collection__item ui-interactive card-hover card-hover--spotlight" href="&#x2F;skip&#x2F;"[^>]*>(.*?)<\/a>/);
  assert.ok(iconlessItem);
  assert.doesNotMatch(iconlessItem[1], /ui-icon/);
});
