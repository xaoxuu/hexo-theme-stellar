...
function addFeature(entries, id, enabled, when, config) {
  if (!enabled) return;
  entries.push({
    id,
    module: "/js/runtime/extensions/feature.mjs",
    when,
...
function buildBrowserRuntimeManifest(input) {
  ...
  if (typeof search.provider === "string" && search.provider.length > 0) {
    ...
    entries.push({
      id: "search",
      module: "/js/runtime/extensions/search.mjs",
  ...
  addFeature(entries, "lazy-loading", true, {
  ...
  entries.push({
    id: "services",
    module: "/js/runtime/extensions/services.mjs",
    when: {
  ...
  if (typeof comments.service === "string" && comments.service.length > 0) {
    entries.push({
      id: "comments",
      module: "/js/runtime/extensions/comments.mjs",
  ...
  addFeature(entries, "preload", features.preload?.enabled === true, { always: true }, {
    asset: assets.features?.preload || null
  });
  addFeature(entries, "lightbox", features.lightbox?.enabled === true,
  ...
  addFeature(entries, "reveal", features.reveal?.enabled === true, { selector: ".slide-up" },
  ...
  if (mathProvider === "mathjax") {
    addFeature(entries, "mathjax", true, { selector: ".has-jax, script[type^='math/tex']" }, {
  ...
  addFeature(entries, "diagrams", diagramsEnabled, { selector: ".mermaid" }, Object.assign({}, features.diagrams, diagramOptions, {
  ...
  addFeature(entries, "swiper", true, { selector: "#swiper-api" }, { assets: assets.features?.swiper || {} });
