 ...
 *
 * 行为：
 * - src 为 GitHub raw 类地址时，origin 替换为调用方传入的 rawUrl；
 *   未传 rawUrl 时保留 src 原 origin；
...
/**
 * 解析 GitHub raw 地址（规范 raw host 或调用方传入的 rawUrl 镜像 origin）。
 * 返回 { host, owner, repo, ref, rest }；无法识别时返回 null。
 */
function rawOrigin(value) {
...
function parseGitHubRaw(url, rawUrl) {
  ...
  const configuredOrigin = rawOrigin(rawUrl);
  if (host !== GITHUB_RAW_HOST && u.origin !== configuredOrigin) {
    return null;
  }
 ...
 * options: { id, replace, heading, rawUrl }
 */
function mdrenderHtml(src, options = {}) {
  const configuredRawUrl = options.rawUrl;
  const parsed = parseGitHubRaw(src, configuredRawUrl);
  let url = src;
  let base = null;
  if (parsed) {
    const origin = rawOrigin(configuredRawUrl) || `https://${parsed.host}`;
    const dir = `${parsed.owner}/${parsed.repo}/${parsed.ref}/`;
    url = `${origin}/${dir}${parsed.rest}`;
    base = `${origin}/${dir}`;
  }
