...
- 凡方案或最终 diff 为旧输入增加别名、双读、fallback、legacy adapter、专用拒绝/迁移诊断，或在测试、文档中点名当前契约不存在的名称，均按兼容处理，不依赖任务名称判断。
- 每项专用兼容必须以发布基线树中的实际输入，或有 Issue/文档与当前消费者佐证的明确外部接缝为来源；未发布候选使用当前通用解析、拒绝和验证路径。
- 替换未发布候选时，以旧标识符全树检查默认配置、Schema、模型投影、模板与客户端消费者、测试和知识库，清理其专用生产者、消费者、诊断、断言和迁移说明。

完成条件：最终 diff 中每项专用兼容都有可复核来源；每个被替换的未发布标识符在当前树中没有专用语义残留。
