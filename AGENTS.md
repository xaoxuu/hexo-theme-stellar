...
- 源码、Schema、生成 Reference 与测试是当前候选的事实来源；知识库是发布快照和探索索引，在能缩小源码搜索范围时按需读取并回到事实来源确认
- 普通开发不刷新知识库；发版准备按 §5 集中同步最终净变化，`VERIFICATION.md` 每个版本汇总一次；纯文档任务和事实纠错即时处理
- 修改知识库后运行 `npm run knowledge:check`；仓库内链接、公开主题配置引用和当前版本引用必须零错误
...
**发布基线**：兼容、迁移、公开文档和回归只面向最近已公开发布的版本及明确承诺维护的外部接缝。同一发布周期内的内部方案、预览、未发布 tarball 和中间提交属于可替换候选；后续实现直接形成新的最终候选，不为被替换候选增加别名、双读、迁移或兼容测试。

**方案门禁**：跨域架构、迁移或兼容取舍、分发/发布工作、需长期保留的产品决策，以及用户明确要求方案的任务，先在 `docs/designs/{YYYY-MM-DD}-{功能简称}/` 写方案。局部修复、样式微调、单字段配置和文档维护直接实现；已有方案时原地表达当前最终候选，不新建重复记录或追加候选演变流水。方案只记录环境无法自证的决策、边界与验收标准，过程历史由 Git 保存。
...
普通开发提交以实现、测试、生成 Reference 与必要方案完成交付；Schema 生成 Reference、Contribution descriptor 的文档路径等机器契约仍随实现通过对应检查。用户要求发版或明确要求文档维护时，按 `docs/guides/release-process.md` 执行净变化文档同步。

完成条件：每项最终净变化均有与实现一致的文档落点或 N/A 结论；文档只描述发布基线到最终候选，不保留被替换候选的兼容路径。

**提交门禁**：

- **开发提交**：同一需求的实现、测试、生成 Reference 与必要方案属于一个交付单位；知识库、公开 Wiki、CHANGELOG 和版本级验证记录留到发版准备集中同步
...
5. `docs/` — 触发方案门禁时维护当前最终候选；发版准备时同步知识库
6. `languages/` — 国际化文案（如需新增文本）

## 6. 架构总览

跨域架构或无法定位影响范围时，可用 `docs/knowledge/00-总览与安装配置/overview.md` 缩小探索范围，再以源码、Schema、生成 Reference 和测试确认当前候选。局部任务直接读取受影响事实来源。
...
发版一键全自动：AI/人工先按发布基线完成净变化文档同步并准备 CHANGELOG，Node 脚本校验非空、更新版本号并推送 → CI 自动完成 npm 发布、tag 创建与 GitHub Release。发版前脚本自动执行 `npm run release:check`，任一失败即中止。
...
- **版本号推导**（比较发布基线树与最终候选树的净变化）：仅含 fix / perf / style → `x.y.(z+1)`；含 feat / refactor / breaking change → `x.(y+1).0`；大型重构、用户可感知的设计调整 → `(x+1).0.0`；测试版本 → `x.y.z-rc.N`
- **文档**：按 `docs/guides/release-process.md` 完成净变化文档同步
