...
当前开发版将站点级客户端配置与图标数据外置到带内容版本的 js/stellar-icons.js，图标 CSS 变量外置到 css/stellar-icons.css；页面语言和永久链接仍由 defines.ejs 提供。颜色算法使用 color.js，图片读取由 runtime/image-color.js 负责；生成期颜色元数据优先，缺失时才从图片计算。pin-slider 已由独立页面级 Extension 管理，不再在 EJS 中内联运行脚本。
