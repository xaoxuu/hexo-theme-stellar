...
| 2026-08-13 | `scripts/lib/mdrender_html.js`、`scripts/helpers/mdrender.js`、`scripts/tags/lib/md.js`、`layout/_partial/widgets/markdown.ejs`、`layout/page.ejs`、`source/js/services/mdrender.js`、`docs/knowledge/03-内容系统/wiki-docs.md`、`docs/designs/2026-08-13-mdrender-abstraction/` | 远程 MD 渲染能力抽象：`{% md %}` 与 markdown 组件复用统一占位生成器（`data-replace` 原地替换 / `data-heading` 标题适配，默认开启；GitHub raw src 自动替换 `api_host.ghraw` 镜像并输出 `data-base`，相对图片/链接解析到镜像基址）；wiki 项目首页正文为空（trim 后）且配置 `repo` 时该页正文渲染为仓库 README.md，页面走既有 `page.ejs` → `layout.ejs` 链路 |
