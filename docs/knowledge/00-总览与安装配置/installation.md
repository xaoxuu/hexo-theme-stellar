...
`2.0.0-rc.6` 使用 npm 的 `rc` dist-tag；不带版本的安装命令仍读取 `latest`，因此在 v2 稳定版发布前会得到 1.x。体验候选版时使用 `npm install hexo-theme-stellar@rc`，锁定部署或复现问题时使用 `npm install hexo-theme-stellar@2.0.0-rc.6`。公开 v2 Wiki 不应被当作 1.x 配置参考；升级时以实际安装版本、Release、[CHANGELOG](../../../CHANGELOG.md)和迁移说明为准。同一个站点不要同时保留 `themes/stellar` 源码和 npm 主题包。
...
`package.json` 是主题版本唯一所有者，`package-lock.json` 与安装知识库由发布流程同步。当前候选版本为 `2.0.0-rc.6`；所有公开版本使用相同的 npm 安装与 Release 流程。
