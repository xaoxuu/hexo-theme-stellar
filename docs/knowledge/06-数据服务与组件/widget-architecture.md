...
`compact` 用于 recent、related、文档树、标签树和 dropdown 等紧凑列表；menubar、用户 linklist 和普通集合使用默认 `auto`。其它 density 值（包括旧 `regular`）统一回退为 `auto`。menubar 仍可在自身组件作用域覆盖条目间距，不改变 collection 的 density 契约。
