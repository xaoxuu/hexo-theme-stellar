...
页面或集合在 `sidebar.left.brand` 中覆盖。Brand 根字段逐项合并，但 `image` 是不可拆分的原子对象；一旦覆盖图片，必须同时提供 `src` 和 `style`。

| 字段 | 语义 |
| --- | --- |
| `image.src` | 图片资源 |
| `image.style` | `avatar`、`icon` 或 `plain` |
| `image.url` | 点击图片后的链接 |
...
Brand 与菜单是两个独立组件。`navigation.menu` 选择 `menubar` 配置，`navigation.breadcrumb` 控制正文面包屑；菜单项的图标仍使用通用 `icon()` helper，与 `brand.image` 无关。
