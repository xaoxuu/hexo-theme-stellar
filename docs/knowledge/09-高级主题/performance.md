...
结果记录在 [reference/v2-alpha-performance.json](../../../reference/v2-alpha-performance.json)：v1 为 35,184 bytes，`2.0.0-alpha.1` 候选 tarball 为 18,811 bytes，降低 46.5354%，通过至少 30% 的门禁。`npm run alpha:performance` 会重新构建两边并逐字节检查记录，已纳入 `npm run check`。
