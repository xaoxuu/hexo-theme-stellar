# Stellar 主题中文知识库（全量）> 合并版：包含全部领域页面，供 RAG 或一次性上下文导入。以 themes/stellar/ 源码（1.41.0）为事实来源。> 注意：各页面内的相对链接按原文件位置计算，本合并版中请按标题检索。---
title: 配置系统
domain: 总览与安装配置
tags:
  - 配置
...
**参考源码**：**参考源码**：[source/css/_custom.styl](../../../source/css/_custom.styl)、[source/css/comments/](../../../source/css/comments/)
...
**参考源码**：[source/css/_components/sidebar/sidebar.styl](../../../source/css/_components/sidebar/sidebar.styl)、**参考源码**：[source/css/_custom.styl](../../../source/css/_custom.styl)、[source/css/comments/](../../../source/css/comments/)
...
flowchart TD
    ...
    WIKIFULL --> END["Output to <title> tag"]
    WIKIHOME --> END
    PAGETITLE --> END
    PAGETITLE2 --> END
    CATTITLE --> END
...
- **自动特性**：
  ...
  - 顶部横幅覆盖层：`page.banner`（或 `banner_info` 关联配置）提供图片时，页面顶部横幅经通用覆盖层 `cover-overlay()`（与文章列表封面、置顶轮播统一；`{% banner %}` 标签不使用该覆盖层）渲染同图渐变模糊层与黑色渐变蒙版（`.banner-mask-top` / `.banner-mask-bottom`）；模糊层与黑色蒙版均**始终显示**（文字所在边缘不透明度约 0.25 → 垂直中线 0，贴满横幅），hover 时背景图与模糊层同步放大至 `scale(1.05)`（图片 1.5s、模糊层 0.5s 缓动）并变暗（亮度 75%、饱和度 120%）。banner 含图片时 `.content` 带 `data-text-adaptive="split"`：标题大字用 contrast（黑白对比，阈值 0.6 偏向浅色文字），面包屑/日期/副标题等小字用 theme（背景图平均色 lighten/darken）；JS 懒加载计算并写入 `--text-banner` / `--text-banner-theme`
...
专栏容器为纯平铺布局（无卡片背景，条目间以分隔线衔接），`.cover` 规则泛化自 photo 卡片，复用 `cover-overlay` 渐变模糊层：
...
**友链文章订阅（`posts:true`）运行时结构：** 数据服务加载后生成 `div.grid-cell.user-post-card`，内含 `div.avatar-box`（头像 + 标题）与 `div.post-panel`（`.desc` 一句话描述 + `.posts` 内多个 `a.post-link`：`span.title` + `span.date`）。`.post-panel` 为公共组件，与专栏卡片右侧文章列表共用。
...
**参考源码**：[_config.yml](../../../_config.yml)、**参考源码**：[source/css/_custom.styl](../../../source/css/_custom.styl)、[source/css/comments/](../../../source/css/comments/)
...
- **设计令牌**：定义于 **参考源码**：[source/css/_custom.styl](../../../source/css/_custom.styl)、[source/css/comments/](../../../source/css/comments/)
...
| 任务 | 扩展点 | 关键文件 | 配置 |
|------|--------|----------|------|
| **改颜色/字体** | 设计令牌 | **参考源码**：[source/css/_custom.styl](../../../source/css/_custom.styl)、[source/css/comments/](../../../source/css/comments/)
...
flowchart TD
  ...
  G --> G1["search_cache_v4 + cache_ttl"]
```
...
本地搜索系统在构建期把全部站点内容序列化为 `/search.json`。客户端缓存带 TTL（`search.local_search.cache_ttl`，默认 `86400` 秒 = 1 天），以 `search_cache_v4` 键写入 `localStorage`（结构 `{ ts, ttl, data }`）：TTL 未过期直接使用缓存、不发请求；过期后先用旧缓存出结果并后台刷新；`cache_ttl: 0` 表示不缓存。
