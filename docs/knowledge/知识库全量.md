...
圆角跟随：Chromium 的 `corner-shape` 只作用于元素自身背景绘制，不传递给子图片的 overflow 裁剪，因此 hero 封面/页顶横幅/`{% banner %}` 的背景图同时由容器自身背景承载（`--cover-url` / `--bg-url`），置顶轮播由插件按当前 slide 把 `--pin-cover-url` 同步到 `.pin-slider` 自身背景、封面 slide 背景透明让容器背景在角落楔形区透出；子图保留用于 hover 滤镜与滑动动画，角落与其它元素一致地跟随配置曲率。
