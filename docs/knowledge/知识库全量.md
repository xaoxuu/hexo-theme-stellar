# Stellar 主题中文知识库（全量）> 合并版：包含全部领域页面，供 RAG 或一次性上下文导入。以 themes/stellar/ 源码（1.38.0）为事实来源。> 注意：各页面内的相对链接按原文件位置计算，本合并版中请按标题检索。---
title: 配置系统
domain: 总览与安装配置
tags:
  - 配置
...
渲染 navbar top 的列表页（首页/归档/标签/分类/专栏/wiki 列表及带 `nav_tabs` 的页面）在导航栏上方自动获得置顶内容轮播位（配置 `pin_slider`，无置顶内容时不渲染）。
...
- 置顶文章在列表页 navbar 上方的置顶轮播中展示（配置 `pin_slider`）
