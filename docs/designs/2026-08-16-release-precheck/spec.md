...
- 成功标准：`npm run check` 全绿；本周期全部非合并提交在 `VERIFICATION.md` 提交登记表登记；已确认的文档缺口修正完毕；发版前漏登记的问题有门禁兜底。
...
- 对外行为：`npm run check` 新增提交登记完整性检查；发版前每个非合并提交须在 `VERIFICATION.md` 登记短 SHA。
