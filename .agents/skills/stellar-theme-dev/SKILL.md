...
description: 主题仓库开发全流程：方案（docs/designs）、代码开发（layout/、scripts/、source/、languages/）、全量验证（npm run check、npm run g && npx gulp minify）、知识库核查（docs/knowledge）、Conventional Commits 提交与主题发版（CHANGELOG + npm run release）。当任务涉及本仓库（hexo-theme-stellar）任何主题文件、主题构建验证、知识库更新或主题发版时使用。
---

# Stellar Theme Dev

主题仓库（hexo-theme-stellar）开发全流程。开始前先读仓库根 `AGENTS.md`（职责与边界）与 `CLAUDE.md`（详细规范）；涉及主题代码、配置或行为问题时，先查 `docs/knowledge/` 对应领域，再读源码确认。
...
- 新增或重构标签插件先读 `docs/guides/tag-plugins-style-guide.md`；代码遵循 `CLAUDE.md` 的 EJS / Node / Stylus / 浏览器 JS 规范。
- 完成条件：改动文件清单覆盖全部相关维度，与知识库和源码一致。

### 3. 验证

- 静态检查与测试：`npm run check`（lint + 单测 + 知识库核查）；新增或修改纯函数时补充单测。
- 全量验证（修改 `scripts/` 后必须）：`npm run g && npx gulp minify`。
...
- 详见 `docs/guides/release-process.md` 与 `CLAUDE.md`「发版规范」。
- 完成条件：版本号与变更摘要经用户确认，dry-run 通过后执行正式发版。

## 护栏

- 修改 `scripts/` 后未通过 `npm run g && npx gulp minify` 不得提交。
